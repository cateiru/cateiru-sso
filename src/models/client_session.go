...
var ClientSessionTableColumns = struct {
	...
	UpdatedAt: "client_session.updated_at",
}
...
var ClientSessionWhere = struct {
	...
	UpdatedAt: whereHelpertime_Time{field: "`client_session`.`updated_at`"},
}
...
var ClientSessionRels = struct {
	User                   string
	Client                 string
	SessionClientRefreshes string
}{
	User:                   "User",
	Client:                 "Client",
	SessionClientRefreshes: "SessionClientRefreshes",
}
...
type clientSessionR struct {
	...
	SessionClientRefreshes ClientRefreshSlice `boil:"SessionClientRefreshes" json:"SessionClientRefreshes" toml:"SessionClientRefreshes" yaml:"SessionClientRefreshes"`
}
...
func (r *clientSessionR) GetClient() *Client {
	if r == nil {
		return nil
	}
	return r.Client
...
func (r *clientSessionR) GetSessionClientRefreshes() ClientRefreshSlice {
	if r == nil {
		return nil
	}
	return r.SessionClientRefreshes
}
...
var (
	clientSessionAllColumns            = []string{"id", "user_id", "client_id", "period", "created_at", "updated_at"}
	clientSessionColumnsWithoutDefault = []string{"id", "user_id", "client_id"}
	clientSessionColumnsWithDefault    = []string{"period", "created_at", "updated_at"}
...
func (o *ClientSession) Client(mods ...qm.QueryMod) clientQuery {
	queryMods := []qm.QueryMod{
...
// SessionClientRefreshes retrieves all the client_refresh's ClientRefreshes with an executor via session_id column.
func (o *ClientSession) SessionClientRefreshes(mods ...qm.QueryMod) clientRefreshQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`client_refresh`.`session_id`=?", o.ID),
	)
...
// loaded structs of the objects. This is for an N-1 relationship.
func (clientSessionL) LoadClient(ctx context.Context, e boil.ContextExecutor, singular bool, maybeClientSession interface{}, mods queries.Applicator) error {
	var slice []*ClientSession
	var object *ClientSession
...
// LoadSessionClientRefreshes allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (clientSessionL) LoadSessionClientRefreshes(ctx context.Context, e boil.ContextExecutor, singular bool, maybeClientSession interface{}, mods queries.Applicator) error {
	var slice []*ClientSession
	var object *ClientSession
	...
	query := NewQuery(
		...
		qm.WhereIn(`client_refresh.session_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.SessionClientRefreshes = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.SessionID {
				local.R.SessionClientRefreshes = append(local.R.SessionClientRefreshes, foreign)
				if foreign.R == nil {
					foreign.R = &clientRefreshR{}
				}
...
// SetClient of the clientSession to the related item.
// Sets o.R.Client to related.
// Adds o to related.R.ClientSessions.
func (o *ClientSession) SetClient(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Client) error {
	var err error
	if insert {
		if err = related.Insert(ctx, exec, boil.Infer()); err != nil {
			return errors.Wrap(err, "failed to insert into foreign table")
...
// AddSessionClientRefreshes adds the given related objects to the existing relationships
// of the client_session, optionally inserting them as new records.
// Appends related to o.R.SessionClientRefreshes.
// Sets related.R.Session appropriately.
func (o *ClientSession) AddSessionClientRefreshes(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*ClientRefresh) error {
	var err error
	...
	if o.R == nil {
		o.R = &clientSessionR{
			SessionClientRefreshes: related,
		}
	} else {
		o.R.SessionClientRefreshes = append(o.R.SessionClientRefreshes, related...)
	}
