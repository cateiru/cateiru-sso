...
func testEmailVerifySessionToOneUserUsingUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()

	var local EmailVerifySession
	var foreign User

	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, emailVerifySessionDBTypes, false, emailVerifySessionColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize EmailVerifySession struct: %s", err)
	}
	...
	slice := EmailVerifySessionSlice{&local}
	if err = local.L.LoadUser(ctx, tx, false, (*[]*EmailVerifySession)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.User == nil {
...
func testEmailVerifySessionToOneSetOpUserUsingUser(t *testing.T) {
	var err error
	...
	var a EmailVerifySession
	var b, c User

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, emailVerifySessionDBTypes, false, strmangle.SetComplement(emailVerifySessionPrimaryKeyColumns, emailVerifySessionColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range []*User{&b, &c} {
		...
		if x.R.EmailVerifySessions[0] != &a {
...
var (
	emailVerifySessionDBTypes = map[string]string{`ID`: `varchar`, `UserID`: `varchar`, `NewEmail`: `varchar`, `VerifyCode`: `char`, `Period`: `datetime`, `RetryCount`: `tinyint`, `CreatedAt`: `datetime`, `ModifiedAt`: `datetime`}
	_                         = bytes.MinRead
)
