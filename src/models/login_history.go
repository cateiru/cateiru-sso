...
func (r *loginHistoryR) GetUser() *User {
	if r == nil {
		return nil
	}
	return r.User
...
func (o *LoginHistory) User(mods ...qm.QueryMod) userQuery {
...
func (loginHistoryL) LoadUser(ctx context.Context, e boil.ContextExecutor, singular bool, maybeLoginHistory interface{}, mods queries.Applicator) error {
	var slice []*LoginHistory
	var object *LoginHistory

	if singular {
		var ok bool
		object, ok = maybeLoginHistory.(*LoginHistory)
		if !ok {
			object = new(LoginHistory)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeLoginHistory)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeLoginHistory))
			}
		}
	} else {
		s, ok := maybeLoginHistory.(*[]*LoginHistory)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeLoginHistory)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeLoginHistory))
			}
		}
	}
	...
	if singular {
		...
		foreign.R.LoginHistories = append(foreign.R.LoginHistories, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.UserID == foreign.ID {
				...
				foreign.R.LoginHistories = append(foreign.R.LoginHistories, local)
				break
			}
		}
	}
...
// SetUser of the loginHistory to the related item.
// Sets o.R.User to related.
// Adds o to related.R.LoginHistories.
func (o *LoginHistory) SetUser(ctx context.Context, exec boil.ContextExecutor, insert bool, related *User) error {
	var err error
	if insert {
	...
	if related.R == nil {
		related.R = &userR{
			LoginHistories: LoginHistorySlice{o},
		}
	} else {
		related.R.LoginHistories = append(related.R.LoginHistories, o)
	}
