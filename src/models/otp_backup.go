...
func (r *otpBackupR) GetUser() *User {
	if r == nil {
		return nil
	}
	return r.User
...
func (o *OtpBackup) User(mods ...qm.QueryMod) userQuery {
...
func (otpBackupL) LoadUser(ctx context.Context, e boil.ContextExecutor, singular bool, maybeOtpBackup interface{}, mods queries.Applicator) error {
	var slice []*OtpBackup
	var object *OtpBackup

	if singular {
		var ok bool
		object, ok = maybeOtpBackup.(*OtpBackup)
		if !ok {
			object = new(OtpBackup)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeOtpBackup)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeOtpBackup))
			}
		}
	} else {
		s, ok := maybeOtpBackup.(*[]*OtpBackup)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeOtpBackup)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeOtpBackup))
			}
		}
	}
	...
	if singular {
		...
		foreign.R.OtpBackups = append(foreign.R.OtpBackups, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.UserID == foreign.ID {
				...
				foreign.R.OtpBackups = append(foreign.R.OtpBackups, local)
				break
			}
		}
	}
...
// SetUser of the otpBackup to the related item.
// Sets o.R.User to related.
// Adds o to related.R.OtpBackups.
func (o *OtpBackup) SetUser(ctx context.Context, exec boil.ContextExecutor, insert bool, related *User) error {
	var err error
	if insert {
	...
	if related.R == nil {
		related.R = &userR{
			OtpBackups: OtpBackupSlice{o},
		}
	} else {
		related.R.OtpBackups = append(related.R.OtpBackups, o)
	}
