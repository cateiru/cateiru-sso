...
func testOtpToOneUserUsingUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, otpDBTypes, false, otpColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize Otp struct: %s", err)
	}
	...
	slice := OtpSlice{&local}
	if err = local.L.LoadUser(ctx, tx, false, (*[]*Otp)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.User == nil {
...
func testOtpToOneSetOpUserUsingUser(t *testing.T) {
	var err error
	...
	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, otpDBTypes, false, strmangle.SetComplement(otpPrimaryKeyColumns, otpColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range []*User{&b, &c} {
		...
		if exists, err := OtpExists(ctx, tx, a.UserID); err != nil {
			t.Fatal(err)
		} else if !exists {
...
var (
	otpDBTypes = map[string]string{`UserID`: `varchar`, `Secret`: `text`, `CreatedAt`: `datetime`, `UpdatedAt`: `datetime`}
	_          = bytes.MinRead
)
